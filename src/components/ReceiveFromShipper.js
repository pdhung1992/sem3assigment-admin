...
const ReceiveFromShipper = () => {
    ...
    const [rcvBills, setRcvBills] = useState([]);
    const fetchRcvBills = async () => {
        const rcv = await billService.rcvFromShipper(axiosConfig);
        setRcvBills(rcv);
    }
    ...
    const handleCreateStatus = async () => {
        ...
        const typeId = emp.empData.role === 'Postman' ? 2 : 3;
        console.log(typeId)
        ...
        try {
            const result = await Swal.fire({
                title: 'Are you sure?',
                text: 'Check the shipment carefully and make sure it no has any irregular, and collect fee from shipper if have!',
                icon: 'warning',
            ...
            Swal.fire({
                title: 'Receive error!',
                text: 'An error occurred while receiving shipment!',
                icon: 'error',
    ...
    return(
        <div className='db-content'>
            <h3>Receive shipment from Shipper:</h3>
            <hr/>
            <Row>
                <Col span={8} style={{padding: "0 1% 0 0"}}>
                    <Space direction="vertical" size="middle" style={{ display: 'flex' }}>
                        <Card headStyle={{background: "#dcdcdc"}} type={'inner'} title={'Find bill by number: '}>
                            <Space direction="vertical" size="middle" style={{ display: 'flex' }}>
                                <Search
                                    placeholder="Enter bill number to receive..."
                                    allowClear
                                    enterButton={
                                        <Button
                        ...
                        <Card headStyle={{background: "#dcdcdc"}} type={'inner'} title={'Bills waiting to receive: '}>
                            <table className="table table-striped">
                                <tbody>
                                {rcvBills.length !== 0 ? (
                                    rcvBills.map((bill, index) => (
                                            <tr onClick={() => setBillDetails(bill)} key={index}>
                                                <td>{bill.billNumber}</td>
                                                <td>{bill.shippingAddressDto.address}, {bill.shippingAddressDto.wardName}</td>
                                            </tr>
                                        ))
                                ) : (
                            ...
                            <div>
                                <Space direction="vertical" size="middle" style={{ display: 'flex' }}>
                                    ...
                                    <div className={"text-center"}>
                                        {billDetails.latestStatus.typeId === 1 ? (
                                            <button className={"btn main-btn"} onClick={handleCreateStatus}>Receive</button>
                                        ) : (
                                            emp.empData.role === 'Staff' && billDetails.latestStatus.typeId === 2 ? (
                                                <button className={"btn main-btn"} onClick={handleCreateStatus}>Receive</button>
                                            ) : (
                                                <button className={"btn main-btn"} disabled style={{color: "#ee0033"}}>Shipment has been received</button>
                                            )
                                        )}
...
export default ReceiveFromShipper;